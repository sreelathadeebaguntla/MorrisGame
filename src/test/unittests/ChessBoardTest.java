package test.unittests;

import static org.junit.Assert.assertEquals;
import static org.junit.jupiter.api.Assertions.*;

import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;

import model.Chess;
import model.ChessBoard;
import model.Location;
import model.Operation;

class ChessBoardTest {
   ChessBoard cs=new ChessBoard();
	@BeforeEach
	void setUp() throws Exception {
		
	}

	@Test
	void testChessBoard() {
		
	  assertNotNull(cs.chessonboard);
	  assertNotNull(cs.opList);
	  
	}

	@Test
	void testPlacable() {
		Chess c=new Chess();
		c.l=Location.a1;
		Operation op0=new Operation();
		op0.lstart=Location.a1;
		Operation op1=new Operation();
		op1.lstart=Location.a4;
	cs.chessonboard.add(c);
	assertFalse(cs.placable(op0));
	assertTrue(cs.placable(op1));
	}

	@Test
	void testPlace() {
		int size= cs.opList.size();
		Operation op0=new Operation();
		op0.lstart=Location.a1;
		cs.place(op0);
		assertEquals(size+1,cs.opList.size());
		
	}

	@Test
	void testMovable() {

		Operation op0=new Operation();
		op0.lstart=Location.a1;
		cs.place(op0);
		Operation op1=new Operation();
		op1.lstart=Location.a4;
		cs.place(op1);
		Operation op2=new Operation();
		op2.lstart=Location.a4;
		op2.lfinish=Location.a1;
		assertFalse(cs.movable(op2));
		
		op2.lfinish=Location.a7;
		assertTrue(cs.movable(op2));
		
		
	}

	@Test
	void testFlyable() {
		Operation op0=new Operation();
		op0.lstart=Location.a1;
		cs.place(op0);
		Operation op1=new Operation();
		op1.lstart=Location.a4;
		cs.place(op1);
		Operation op2=new Operation();
		assertFalse(cs.flyable(op2));
		op2.lstart=Location.a7;
		cs.place(op2);
		assertTrue(cs.flyable(op2));

	}

	@Test
	void testMove() {
		int size= cs.opList.size();
		Operation op0=new Operation();
		op0.lstart=Location.a1;
		cs.place(op0);
		Operation op1=new Operation();
		op1.lstart=Location.a1;
		op1.lfinish=Location.a4;
		cs.move(op1);
		assertEquals(size+2,cs.opList.size());
	}

	@Test
	void testHasMill() {
		Operation op0=new Operation();
		op0.lstart=Location.a1;
		cs.place(op0);
		Operation op1=new Operation();
		op1.lstart=Location.a4;
		cs.place(op1);
		assertTrue(cs.hasMill(Location.a7, 0));
		assertFalse(cs.hasMill(Location.d1, 0));
	}

	@Test
	void testEatable() {
		Operation op0=new Operation();
		op0.lstart=Location.a1;
		cs.place(op0);
		Operation op1=new Operation();
		op1.lstart=Location.a4;
		cs.place(op1);
		Operation op2=new Operation();
		op2.lstart=Location.a7;
		cs.place(op2);

		Operation op4=new Operation();
		op4.player=1;
		op4.lstart=Location.a1;
		assertTrue(cs.eatable(op4));
		Operation op3=new Operation();
		op3.lstart=Location.d1;
		cs.place(op3);
		assertFalse(cs.eatable(op4));
		op4.lstart=Location.d1;
		assertTrue(cs.eatable(op4));
	}

	@Test
	void testEat() {
		int size= cs.opList.size();
		Operation op0=new Operation();
		op0.lstart=Location.a1;
		cs.place(op0);
		Operation op4=new Operation();
		op4.player=1;
		op4.lstart=Location.a1;
		cs.eat(op4);
		assertEquals(size+2,cs.opList.size());
	}

	@Test
	void testIsFinishByCount() {
		Operation op0=new Operation();
		op0.lstart=Location.a1;
		cs.place(op0);
		Operation op1=new Operation();
		op1.lstart=Location.a4;
		cs.place(op1);
		Operation op2=new Operation();
		op2.lstart=Location.a7;
		cs.place(op2);
		assertEquals(0,cs.isFinishByCount());
		Operation op3=new Operation();
		op3.lstart=Location.g1;
		op3.player=1;
		cs.place(op3);
		Operation op4=new Operation();
		op4.lstart=Location.g4;
		op4.player=1;
		cs.place(op4);
		Operation op5=new Operation();
		op5.lstart=Location.g7;
		op5.player=1;
		cs.place(op5);
		assertEquals(-1,cs.isFinishByCount());
	}

	@Test
	void testIsFinishByMove() {
		Operation op0=new Operation();
		op0.lstart=Location.a1;
		cs.place(op0);
		Operation op1=new Operation();
		op1.lstart=Location.b4;
		cs.place(op1);
		Operation op3=new Operation();
		op3.lstart=Location.a4;
		op3.player=1;
		cs.place(op3);
		assertEquals(-1,cs.isFinishByMove());
		Operation op2=new Operation();
		op2.lstart=Location.a7;
		cs.place(op2);
		assertEquals(0,cs.isFinishByMove());
	}

}
